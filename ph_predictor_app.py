...
def main(page: ft.Page):
    ...
    def save_image_click(e):
        ...
        if current_png_data:
            ...
            page.snack_bar.open = True; page.update()
    context_menu = ft.Card(content=ft.Column([ft.ListTile(title=ft.Text("画像を保存する", color="black"), on_click=save_image_click, autofocus=True, hover_color="grey_200", width=200)], spacing=0),
    ...
    candidate_dialog = ft.AlertDialog(
        modal=True,
        title=ft.Text("複数の候補が見つかりました"),
        content=ft.Column(spacing=5, height=200, scroll=ft.ScrollMode.ADAPTIVE)
    )

    page.overlay.extend([save_dialog, context_menu, candidate_dialog])
    ...
    def predict_pka(smiles: str):
        ...
        current_predicted_pka = None; current_png_data = None
        try:
        ...
        for key, pattern in COMPILED_PATTERNS.items():
            if pattern and mol.HasSubstructMatch(pattern): found_groups.append(PKa_RULEBOOK[key])
        ...
        with open("temp_mol.png", "wb") as f: f.write(png_data)
        with open("temp_mol.png", "rb") as f: img_base_64 = base64.b64encode(f.read()).decode('utf-8')
    ...
    def handle_predict_click(e):
        ...
        if not identifier:
            result_display.value = "IDを入力してください。"; details_display.value = ""
            ...
            for comp in compounds[:10]:
                candidate_dialog.content.controls.append(
                    ft.ListTile(
                        ...
                        subtitle=ft.Text(comp.molecular_formula or "", size=10),
                        on_click=on_candidate_click,
                        data=comp.connectivity_smiles
                    )
                )
    ...
    id_type_dropdown = ft.Dropdown(label="入力タイプ", width=400, options=[
        ft.dropdown.Option("name", "化合物名 (英語・略称)"), ft.dropdown.Option("smiles", "SMILES"),
    ...
    iupac_name_text, formula_text, weight_text, cas_text, smiles_text, boiling_point_text = ft.Text(selectable=True), ft.Text(selectable=True), ft.Text(selectable=True), ft.Text(selectable=True), ft.Text(selectable=True), ft.Text(selectable=True)
    ...
    error_display = ft.Text(weight=ft.FontWeight.BOLD, selectable=True)
    ...
    image_container = ft.GestureDetector(content=mol_image, on_secondary_tap_down=open_context_menu)
